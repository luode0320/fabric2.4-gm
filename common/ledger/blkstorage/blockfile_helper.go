...
// loadBootstrappingSnapshotInfo 加载引导快照信息(最后一个区块的编号,最后一个区块的哈希值,上一个区块的哈希值)。
// 方法接收者：无（函数）
// 输入参数：
//   - rootDir：根目录路径。
//
// 返回值：
//   - *BootstrappingSnapshotInfo：加载的引导快照信息。
//   - error：如果加载引导快照信息时出错，则返回错误；如果引导快照信息文件不存在，则返回nil。
func loadBootstrappingSnapshotInfo(rootDir string) (*BootstrappingSnapshotInfo, error) {
	...
	// 如果引导快照信息文件不存在，则返回nil
	if os.IsNotExist(err) {
		return nil, nil
	}
	// 如果读取文件内容时出错，则返回错误
	if err != nil {
		return nil, errors.Wrapf(err, "读取 bootstrappingSnapshot.info 文件时出错")
	}

	// 创建一个BootstrappingSnapshotInfo实例, 包含有关引导快照的信息, (最后一个区块的编号,最后一个区块的哈希值,上一个区块的哈希值)
	bsi := &BootstrappingSnapshotInfo{}
	// 将读取的文件内容进行解组（反序列化）
	if err := proto.Unmarshal(bsiBytes, bsi); err != nil {
		return nil, errors.Wrapf(err, "反序列化引导快照信息时出错(最后一个区块的编号,最后一个区块的哈希值,上一个区块的哈希值)")
	}
