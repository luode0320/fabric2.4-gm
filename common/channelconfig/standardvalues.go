...
// DeserializeProtoValuesFromGroup 从配置组中反序列化所有值的值。
// 方法接收者：
//   - 无
//
// 输入参数：
//   - group：*cb.ConfigGroup，表示配置组
//   - protosStructs：...interface{}，表示一个或多个结构体的可变参数列表
//
// 返回值：
//   - error：表示反序列化过程中可能出现的错误
func DeserializeProtoValuesFromGroup(group *cb.ConfigGroup, protosStructs ...interface{}) error {
	// 创建 StandardValues 实例
	sv, err := NewStandardValues(protosStructs...)
	if err != nil {
		logger.Panicf("这只是一个编译时的错误，proto结构在某种程度上无效: %s", err)
	}

	// 遍历配置组中的每个值，并将其反序列化到相应的结构体中
