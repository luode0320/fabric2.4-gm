...
// IsX509SM2Certificate x509是否是sm2类型的公钥
func IsX509SM2Certificate(cert *x509.Certificate) bool {
	ecdsaPublicKey, ok := cert.PublicKey.(*ecdsa.PublicKey)
	return ok && ecdsaPublicKey.Curve == sm2.P256Sm2()
}

// IsPureGMTLSCertificate gmtls是否是sm2类型的私钥
func IsPureGMTLSCertificate(cert *gmtls.Certificate) bool {
	_, ok := cert.PrivateKey.(*sm2.PrivateKey)
	return ok
}

// CreateCertificateToMem 调用SM2 x509GM 接口生成SM2证书
...
// CreateSm2CertificateRequestToMem 调用SM2 x509GM接口生成SM2证书请求
...
// ParseSm2CertificateRequest2X509 SM2 x509GM证书请求 转换 X509 证书请求
func ParseSm2CertificateRequest2X509(sm2req *x509GM.CertificateRequest) *x509.CertificateRequest {
...
// ParseX509CertificateRequest2Sm2 X509 证书请求转换 SM2 x509GM证书请求
...
// ParseX509Certificate2Sm2 X509证书格式转换为 x509GM SM2 证书格式
func ParseX509Certificate2Sm2(x509Cert *x509.Certificate) *x509GM.Certificate {
	sm2cert := &x509GM.Certificate{
		...
		// MaxPathLenZero表示BasicConstraintsValid = = true和
		// MaxPathLen = = 0应解释为实际最大路径长度
		// 为零。否则，该组合被解释为MaxPathLen
		// 未设置。
...
// ParseSm2Certificate2X509 sm2 x509GM 证书转换 x509 证书
