...
// PeerOperations 包含支持背书者所需的对等操作。
type PeerOperations interface {
	// GetApplicationConfig 获取通道的应用程序配置。它接收一个通道 ID 作为参数，并返回一个 channelconfig.Application 对象和一个布尔值，指示是否找到了该通道的应用程序配置。
	GetApplicationConfig(cid string) (channelconfig.Application, bool)
	// GetLedger 获取通道的账本。它接收一个通道 ID 作为参数，并返回一个 ledger.PeerLedger 对象。
	GetLedger(cid string) ledger.PeerLedger
}

// SupportImpl 提供了 endorser.Support 接口的实现，通过调用对等节点的各种静态方法来发出调用。
type SupportImpl struct {
	*PluginEndorser                                       // PluginEndorser 是使用插件对提案响应进行背书的结构体。
	identity.SignerSerializer                             // SignerSerializer 对 Sign 和 Serialize 方法进行分组。
	Peer                      PeerOperations              // PeerOperations 包含支持背书者所需的对等操作。
	ChaincodeSupport          *chaincode.ChaincodeSupport // ChaincodeSupport 负责与 Peer 上的链码进行接口交互。
	ACLProvider               aclmgmt.ACLProvider         // ACLProvider 是访问控制列表（ACL）提供者。
	BuiltinSCCs               scc.BuiltinSCCs             // BuiltinSCCs 是特殊的系统链码，与其他（插件）系统链码区分开来。
}
...
func (s *SupportImpl) Execute(txParams *ccprovider.TransactionParams, name string, input *pb.ChaincodeInput) (*pb.Response, *pb.ChaincodeEvent, error) {
	...
	// 调用ChaincodeSupport的Execute方法执行链码
