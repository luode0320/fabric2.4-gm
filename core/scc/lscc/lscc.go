...
// SCC 实现了链码的生命周期和相关策略。
type SCC struct {
	ACLProvider        aclmgmt.ACLProvider                   // aclProvider 负责访问控制评估
	BuiltinSCCs        scc.BuiltinSCCs                       // BuiltinSCCs 是内置的系统链码
	SCCProvider        sysccprovider.SystemChaincodeProvider // SCCProvider 是传递给系统链码的接口，用于访问系统的其他部分
	Support            FilesystemSupport                     // Support 提供了几个静态函数的实现
	GetMSPIDs          MSPIDsGetter                          // GetMSPIDs 是获取 MSP ID 的函数
	GetMSPManager      MSPManagerGetter                      // GetMSPManager 是获取 MSP 管理器的函数
	BuildRegistry      *container.BuildRegistry              // BuildRegistry 是构建注册表
	ChaincodeBuilder   ChaincodeBuilder                      // ChaincodeBuilder 是链码构建器
	EbMetadataProvider *externalbuilder.MetadataProvider     // EbMetadataProvider 是外部构建器的元数据提供者
	BCCSP              bccsp.BCCSP                           // BCCSP 实例
	PackageCache       PackageCache                          // PackageCache 是链码包缓存
}

// PeerShim 通过提供之前由 scc 提供者提供的方法，将 peer 实例适配为与 LSCC 一起使用。
// 如果 lscc 代码不会很快被删除，可能值得重写它以直接使用这些 API，而不是通过这个适配器，但它很快就会消失。
type PeerShim struct {
	Peer *peer.Peer // Peer 是 peer 实例
}
