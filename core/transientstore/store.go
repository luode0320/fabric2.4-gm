...
// NewStoreProvider 函数实例化 TransientStoreProvider。
//
// 参数：
//   - path: string 类型，表示存储提供程序的路径。
//
// 返回值：
//   - StoreProvider: 表示 TransientStoreProvider 实例。
//   - error: 表示实例化过程中的错误。
func NewStoreProvider(path string) (StoreProvider, error) {
	// 定义一个临时文件存储, 确保在节点关闭时调用该例程。
	lockPath := filepath.Join(filepath.Dir(path), transientStorageLockName)
	// 一个基于文件的锁管理器
	lock := leveldbhelper.NewFileLock(lockPath)
	if err := lock.Lock(); err != nil {
		return nil, errors.WithMessage(err, "当另一个对等节点命令正在执行时,"+
			" 等待该命令完成其执行或在重试之前终止它")
	}

	// 在持有 newStoreProvider 文件锁的情况下，必须调用此例程。
	// 创建leverdb存储目录, 并启动leveldb
	provider, err := newStoreProvider(path, lock)
	if err != nil {
		lock.Unlock()
		return nil, errors.WithMessagef(err, "无法在文件夹中构建存储提供程序 [%s]", path)
	}

	return provider, nil
}

// newStoreProvider 是一个私有方法，用于解开包级别的 Drop 和 NewStoreProvider 例程之间的依赖关系。
// 在持有 newStoreProvider 文件锁的情况下，必须调用此例程。
//
// 参数：
//   - providerPath: string 类型，表示提供程序的路径。
//   - fileLock: *leveldbhelper.FileLock 类型，表示文件锁。
//
// 返回值：
//   - *storeProvider: 表示 storeProvider 实例。
//   - error: 表示实例化过程中的错误。
func newStoreProvider(providerPath string, fileLock *leveldbhelper.FileLock) (*storeProvider, error) {
	...
	if err != nil {
		return nil, errors.WithMessage(err, "无法启动leveldb")
	}

	provider := &storeProvider{dbProvider: dbProvider, fileLock: fileLock}

	// 清除任何标记为删除的数据库。这可能发生在临时存储删除失败后的下一个对等机初始化之后，原因是崩溃或系统错误。
	if err = provider.processPendingStorageDeletions(); err != nil {
		return nil, errors.WithMessagef(err, "正在处理文件夹中挂起的存储删除 [%s]", providerPath)
	}
...
// processPendingStorageDeletions 函数删除所有标记为 UNDER_DELETION 的临时存储。
// 此例程可以在提供程序构造时调用，以清除任何部分删除的临时存储。
//
// 返回值：
//   - error: 表示处理过程中的错误。
func (provider *storeProvider) processPendingStorageDeletions() error {
	dl, err := provider.getStorageMarkedForDeletion()
	if err != nil {
		return errors.WithMessage(err, "正在处理挂起的删除列表")
	}

	for _, l := range dl.List {
		err = provider.deleteStore(l)
		if err != nil {
			return errors.WithMessagef(err, "正在处理用于存储的删除 [%s]", l)
		}

		err = provider.clearStorageDeletionStatus(l)
		if err != nil {
			return errors.WithMessagef(err, "清除存储的删除状态 [%s]", l)
		}
	}
