...
type Registry interface {
	Register(*Handler) error // 注册处理程序
	Ready(string)            // 标记为就绪
	Failed(string, error)    // 标记为失败
	Deregister(string) error // 注销处理程序
}
...
// handleMessage 被 ProcessStream 调用以分派消息。
// 方法接收者：h（Handler类型的指针）
// 输入参数：
//   - msg：ChaincodeMessage类型的指针，表示要处理的消息。
//
// 返回值：
//   - error：如果处理消息时出错，则返回错误。
func (h *Handler) handleMessage(msg *pb.ChaincodeMessage) error {
	// 记录日志，表示处理 ChaincodeMessage 类型的消息
	chaincodeLogger.Debugf("[%s] 类型为 %s 的处理链码消息, 状态为 %s", shorttxid(msg.Txid), msg.Type, h.state)

	// 如果消息类型是 KEEPALIVE，则直接返回
	...
	default:
		return errors.Errorf("处理消息: 状态 %s (对于事务 %s) 无效", h.state, msg.Txid)
	}
}
...
func (h *Handler) handleMessageCreatedState(msg *pb.ChaincodeMessage) error {
	...
	default:
		return fmt.Errorf("[%s] 处理程序无法处理处于已创建状态的消息 (%s)", msg.Txid, msg.Type)
	}
	return nil
}
...
func (h *Handler) handleMessageReadyState(msg *pb.ChaincodeMessage) error {
	...
	case pb.ChaincodeMessage_GET_STATE:
		// 一个中间件函数，用于在将消息转发给提供的委托函数之前获取和验证事务上下文
...
// HandleTransaction 是一个中间件函数，用于在将消息转发给提供的委托函数之前获取和验证事务上下文。
// 响应消息由委托函数发送到聊天流中。委托函数返回的任何错误都被封装为链码错误消息。
// 方法接收者：*Handler
// 输入参数：
//   - msg：*pb.ChaincodeMessage 类型，表示链码消息。
//   - delegate：handleFunc 类型，表示委托函数。
func (h *Handler) HandleTransaction(msg *pb.ChaincodeMessage, delegate handleFunc) {
	chaincodeLogger.Debugf("[%s] 正在处理来自链码的 %s", shorttxid(msg.Txid), msg.Type.String())
	...
	if msg.Type == pb.ChaincodeMessage_INVOKE_CHAINCODE {
		...
		txContext, err = h.isValidTxSim(msg.ChannelId, msg.Txid, "无分类帐上下文")
	}
	...
	for {
		...
		case rmsg := <-msgAvail:
			...
			case rmsg.err == io.EOF:
				chaincodeLogger.Debugf("已接收EOF, 结束链码流: %s", rmsg.err)
				return rmsg.err
			case rmsg.err != nil:
				err := errors.Wrap(rmsg.err, "从链码流接收消息失败")
				chaincodeLogger.Debugf("%+v", err)
				return err
			case rmsg.msg == nil:
				err := errors.New("收到空消息，结束链码流")
				chaincodeLogger.Debugf("%+v", err)
				return err
			default:
				// 调用handleMessage注册消息类型, 以分派消息
				err := h.handleMessage(rmsg.msg)
				if err != nil {
					err = errors.WithMessage(err, "错误处理消息, 结束流")
		...
		case sendErr := <-h.errChan:
			err := errors.Wrapf(sendErr, "发送消息时收到错误, 结束链码流")
			chaincodeLogger.Errorf("%s", err)
			return err
		case <-keepaliveCh:
			// 如果没有来自serialSend的错误消息，请保持活跃，并且不在乎错误
			// (也许以后会起作用)
...
// notifyRegistry 在注册成功时发送 ready 消息，并更新处理程序注册表中链码的启动状态。
// 方法接收者：h（Handler类型的指针）
// 输入参数：
//   - err：错误，表示注册过程中可能发生的错误。
func (h *Handler) notifyRegistry(err error) {
	...
	// 如果发送 ready 消息时出错，则将链码标记为失败，并记录错误日志
	if err != nil {
		h.Registry.Failed(h.chaincodeID, err)
		chaincodeLogger.Errorf("启动失败 %s -- %s", h.chaincodeID, err)
		return
	}
...
// HandleRegister 在chaincode尝试注册时调用handleRegister。
func (h *Handler) HandleRegister(msg *pb.ChaincodeMessage) {
	chaincodeLogger.Debugf("已收到消息类型为 %s, 状态为 %s", msg.Type, h.state)
	chaincodeID := &pb.ChaincodeID{}
	err := proto.Unmarshal(msg.Payload, chaincodeID)
	if err != nil {
		chaincodeLogger.Errorf("接收到的 %s 中出错, 无法取反序列化注册信息: %s", pb.ChaincodeMessage_REGISTER, err)
		return
	}

	// 现在向chaincodeSupport注册
	// 注意: chaincodeID.Name的形式实际上是name:version (对于较旧的链代码)，并且
	// 形式的标签: 哈希为较新的链码。无论哪种方式，它都是句柄
	// 我们跟踪链代码的注册。
	if chaincodeID.Name == "" {
		h.notifyRegistry(errors.New("处理注册链码时出错, 链码id名称为空"))
		return
	}
	h.chaincodeID = chaincodeID.Name
	err = h.Registry.Register(h)
	if err != nil {
		// 在注册成功时发送 ready 消息，并更新处理程序注册表中链码的启动状态
		h.notifyRegistry(err)
		return
	}

	chaincodeLogger.Debugf("已获得 %s (对于chaincodeID = %s)，正在发送回 % s", pb.ChaincodeMessage_REGISTER, h.chaincodeID, pb.ChaincodeMessage_REGISTERED)
	if err := h.serialSend(&pb.ChaincodeMessage{Type: pb.ChaincodeMessage_REGISTERED}); err != nil {
		chaincodeLogger.Errorf("发送错误 %s: %s", pb.ChaincodeMessage_REGISTERED, err)
		h.notifyRegistry(err)
		return
	}
	...
	chaincodeLogger.Debugf("已将状态更改为已建立 (用于 %s)", h.chaincodeID)
...
// 方法接收者：h（Handler类型的指针）
// 输入参数：
//   - msg：pb.ChaincodeMessage类型，表示链码消息。
func (h *Handler) Notify(msg *pb.ChaincodeMessage) {
	// 从 TXContexts 中获取与消息相关的上下文
	tctx := h.TXContexts.Get(msg.ChannelId, msg.Txid)
	if tctx == nil {
		chaincodeLogger.Debugf("通知程序 交易txid:%s, channelID:%s 不存在, 无法处理消息 %s", msg.Txid, msg.ChannelId, msg.Type)
		return
	}

	// 记录日志，表示正在通知处理程序处理消息
	chaincodeLogger.Debugf("[%s] 没有错误: %s, channelID:%s", shorttxid(msg.Txid), msg.Txid, msg.ChannelId)
...
func (h *Handler) HandleGetState(msg *pb.ChaincodeMessage, txContext *TransactionContext) (*pb.ChaincodeMessage, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "反序列化 pb.GetState 状态失败")
	}

	var res []byte
	namespaceID := txContext.NamespaceID
	collection := getState.Collection
	chaincodeLogger.Debugf("[%s] 正在获取链码 %s 的状态, key %s, 通道 %s", shorttxid(msg.Txid), namespaceID, getState.Key, txContext.ChannelID)

	if isCollectionSet(collection) {
		if txContext.IsInitTransaction {
			return nil, errors.New("链码 Init() 中不允许使用私有数据api")
		}
	...
	if res == nil {
		chaincodeLogger.Debugf("[%s] 没有与密钥关联的状态: %s. 正在发送负载为空的 %s", shorttxid(msg.Txid), getState.Key, pb.ChaincodeMessage_RESPONSE)
	}
...
func (h *Handler) Execute(txParams *ccprovider.TransactionParams, namespace string, msg *pb.ChaincodeMessage, timeout time.Duration) (*pb.ChaincodeMessage, error) {
	...
	case ccresp = <-txctx.ResponseNotifier:
		// 响应发送给用户或调用链码。ChaincodeMessage_ERROR
		// 通常被视为错误
