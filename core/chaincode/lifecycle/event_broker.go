...
// EventBroker 接收来自生命周期缓存的事件，并调用已注册的监听器。
type EventBroker struct {
	chaincodeStore       ChaincodeStore                    // 链码存储，用于访问链码的相关信息
	ebMetadata           *externalbuilder.MetadataProvider // 外部构建器的元数据提供者
	pkgParser            PackageParser                     // 包解析器，用于解析链码包
	defineCallbackStatus *sync.Map                         // 定义回调状态，用于跟踪链码定义回调的状态

	mutex     sync.Mutex                                          // 互斥锁，用于保护共享资源的访问
	listeners map[string][]ledger.ChaincodeLifecycleEventListener // 监听器映射，存储已注册的链码生命周期事件监听器
}
