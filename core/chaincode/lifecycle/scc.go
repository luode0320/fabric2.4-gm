...
// ChannelConfigSource 提供了一种获取给定通道ID的通道配置的方法。
type ChannelConfigSource interface {
	// GetStableChannelConfig 返回给定通道ID的通道配置。
	// 注意，它是一个稳定的配置包，即使通道发生变化，它也不会被更新，因此在使用后应该丢弃。
...
// 它将调用路由到后端实现。
type SCC struct {
	OrgMSPID               string                               // 组织的 MSP ID
	ACLProvider            aclmgmt.ACLProvider                  // ACL 提供者
	ChannelConfigSource    ChannelConfigSource                  // 通道配置来源
	DeployedCCInfoProvider ledger.DeployedChaincodeInfoProvider // 已部署链码信息提供者
	QueryExecutorProvider  QueryExecutorProvider                // 查询执行器提供者
	Functions              SCCFunctions                         // lifecycle 的后端实现
...
func (scc *SCC) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	args := stub.GetArgs()
	if len(args) == 0 {
		return shim.Error("必须使用参数调用生命周期scc")
	}

	if len(args) != 2 {
		return shim.Error(fmt.Sprintf("生命周期scc操作只需要两个参数, 但已收到 %d", len(args)))
	}
	...
	if channelID = stub.GetChannelID(); channelID != "" {
		...
		if channelConfig == nil {
			return shim.Error(fmt.Sprintf("无法获取通道 '%s'的 channelconfig 配置", channelID))
		}
		var ok bool
		ac, ok = channelConfig.ApplicationConfig()
		if !ok {
			return shim.Error(fmt.Sprintf("无法获取通道 '%s' 的应用程序配置", channelID))
		}
		if !ac.Capabilities().LifecycleV20() {
			return shim.Error(fmt.Sprintf("无法为通道 '%s' 使用新生命周期, 因为它没有启用所需的功能", channelID))
		}
	}
	...
	if err != nil {
		return shim.Error(fmt.Sprintf("由于 ACL 权限检查失败, 无法授权调用: %s", err))
	}
	...
	if err != nil {
		...
		default:
			return shim.Error(fmt.Sprintf("调用失败 '%s': %s", string(args[0]), err.Error()))
		}
	}
