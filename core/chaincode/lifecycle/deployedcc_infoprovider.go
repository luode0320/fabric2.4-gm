...
// ValidatorCommitter 是验证器和提交者的结构体。
type ValidatorCommitter struct {
	CoreConfig                   *peer.Config                 // peer核心配置
	PrivdataConfig               *privdata.PrivdataConfig     // 私有数据配置
	Resources                    *Resources                   // 配置资源
	LegacyDeployedCCInfoProvider LegacyDeployedCCInfoProvider // 遗留部署的链码信息提供者
}
