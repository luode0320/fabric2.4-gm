...
// NewCache 创建一个缓存实例，用于存储链码的相关信息和状态。
//
// 参数：
//   - resources：资源管理器，用于访问链码存储和包解析器等资源。
//   - myOrgMSPID：本地组织的 MSP 标识符。
//   - metadataManager：元数据处理器，用于处理链码的元数据。
//   - custodian：链码保管者，用于管理链码的构建和启动。
//   - ebMetadata：外部构建器的元数据提供者。
//
// 返回值：
//   - *Cache：缓存实例。
func NewCache(resources *Resources, myOrgMSPID string, metadataManager MetadataHandler, custodian *ChaincodeCustodian, ebMetadata *externalbuilder.MetadataProvider) *Cache {
	return &Cache{
		chaincodeCustodian: custodian,                                                                     // 链码保管者，用于管理链码的构建和启动
		definedChaincodes:  map[string]*ChannelCache{},                                                    // 已定义链码的缓存映射
		localChaincodes:    map[string]*LocalChaincode{},                                                  // 本地链码的缓存映射
		Resources:          resources,                                                                     // 资源管理器，用于访问链码存储和包解析器等资源
		MyOrgMSPID:         myOrgMSPID,                                                                    // 本地组织的 MSP 标识符
		eventBroker:        NewEventBroker(resources.ChaincodeStore, resources.PackageParser, ebMetadata), // 事件代理，用于处理链码事件
		MetadataHandler:    metadataManager,                                                               // 元数据处理器，用于处理链码的元数据
	}
}

// InitializeLocalChaincodes 初始化本地链码。应该在缓存创建后调用一次 (时间无关紧要，尽管已经安装的链码在完成之前不会被调用)。
// 理想情况下，这将是构造函数的一部分，但是，我们不能依赖在创建缓存之前创建的链码存储。
func (c *Cache) InitializeLocalChaincodes() error {
	...
	if err != nil {
		return errors.WithMessage(err, "无法列出已安装的链码")
	}

	// 遍历已安装的链码包
	for _, ccPackage := range ccPackages {
		// 加载链码包
		logger.Infof("加载链码包: %s", ccPackage.PackageID)
		ccPackageBytes, err := c.Resources.ChaincodeStore.Load(ccPackage.PackageID)
		if err != nil {
			return errors.WithMessagef(err, "无法加载具有包 PackageID 的链代码 '%s'", ccPackage.PackageID)
		}

		// 解析链码包
		parsedCCPackage, err := c.Resources.PackageParser.Parse(ccPackageBytes)
		if err != nil {
			return errors.WithMessagef(err, "无法解析具有包 PackageID 的链代码 '%s'", ccPackage.PackageID)
		}
	...
	logger.Infof("已初始化生命周期缓存，其中已安装 %d 个链码", len(c.localChaincodes))
	...
	for channelID, chaincodeCache := range c.definedChaincodes {
		...
		logger.Infof("初始化的生命周期缓存，用于通道 '%s' 与 %d 链码运行，(%d 已批准，%d 已安装)", channelID, runnable, approved, installed)
	}
...
func (c *Cache) handleChaincodeInstalledWhileLocked(initializing bool, md *persistence.ChaincodePackageMetadata, packageID string) {
	// 从序列化包中获取这个值会很好，但是如何以一种很好的方式公开这个值并不明显，所以我们手动计算它。
	...
	for channelID, channelCache := range localChaincode.References {
		for chaincodeName, cachedChaincode := range channelCache {
			...
			logger.Infof("已安装的链码包ID为 '%s' 的链码现在可在通道 %s 上使用, 用于链码定义 %s:%s", packageID, channelID, chaincodeName, cachedChaincode.Definition.EndorsementInfo.Version)
			// 通知已安装且可运行
...
func (c *Cache) update(initializing bool, channelID string, dirtyChaincodes map[string]struct{}, qe ledger.SimpleQueryExecutor) error {
	...
	for name := range dirtyChaincodes {
		logger.Infof("正在更新链码 '%s' (在通道 '%s' 上) 的缓存定义", name, channelID)
		...
		cachedChaincode.InstallInfo = localChaincode.Info
		if localChaincode.Info != nil {
			logger.Infof("在通道 %s 的 链码 package ID='%s' 现在可用, 对于链码定义 %s:%s", channelID, localChaincode.Info.PackageID, name, cachedChaincode.Definition.EndorsementInfo.Version)
