...
// Mgr 管理配置的历史记录，例如链码的集合配置。
// 它应该被注册为状态监听器。状态监听器构建历史记录。
type Mgr struct {
	ccInfoProvider ledger.DeployedChaincodeInfoProvider // 链码信息提供者
	dbProvider     *dbProvider                          // 数据库提供者
}

// NewMgr 构造实现接口 "mgr" 的实例。
// 输入参数：
//   - dbPath：字符串，表示数据库路径
//   - ccInfoProvider：ledger.DeployedChaincodeInfoProvider 接口，表示链码信息提供者
//
// 返回值：
//   - *Mgr：表示 "mgr" 实例的指针
