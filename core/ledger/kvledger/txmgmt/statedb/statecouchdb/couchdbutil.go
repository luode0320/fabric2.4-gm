...
// createCouchInstance 创建couchdb实例
func createCouchInstance(config *ledger.CouchDBConfig, metricsProvider metrics.Provider) (*couchInstance, error) {
	...
	couchInstance := &couchInstance{
		conf:   config,                    // CouchDB 配置信息
		client: client,                    // 用于连接到此实例的客户端
		stats:  newStats(metricsProvider), // 统计信息
	}

	// 提供验证连接信息的函数, 验证数据库用户密码和系统表
	...
	if retVal.StatusCode != 200 {
		return nil, errors.Errorf("CouchDB 连接错误, 期望的返回状态码code为200, 已收到 %v", retVal.StatusCode)
	}
...
func createSystemDatabasesIfNotExist(couchInstance *couchInstance) error {
	...
	if err != nil {
		couchdbLogger.Errorf("调用错误CouchDB createDatabaseIfNotExist() 对于系统数据库dbName: %s, error: %s", dbName, err)
		return err
	}
