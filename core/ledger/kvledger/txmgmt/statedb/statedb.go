...
// VersionedDBProvider 提供一个版本化数据库的实例
type VersionedDBProvider interface {
	// GetDBHandle 返回一个 VersionedDB 的句柄
	GetDBHandle(id string, namespaceProvider NamespaceProvider) (VersionedDB, error)
	// ImportFromSnapshot 从先前使用 FullScanIterator 生成的快照文件中加载公共状态和私有数据哈希
	ImportFromSnapshot(id string, savepoint *version.Height, itr FullScanIterator) error
	// BytesKeySupported 如果创建的数据库支持字节作为键，则返回 true
	BytesKeySupported() bool
	// Close 关闭所有 VersionedDB 实例并释放 VersionedDBProvider 持有的任何资源
	Close()
	// Drop 删除通道特定的账本数据
