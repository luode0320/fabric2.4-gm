...
// Initializer 捕获了事务管理器的依赖项。
type Initializer struct {
	LedgerID            string                                         // 账本ID
	DB                  *privacyenabledstate.DB                        // 数据库
	...
	CCInfoProvider      ledger.DeployedChaincodeInfoProvider           // 部署的链码信息提供者
	CustomTxProcessors  map[common.HeaderType]ledger.CustomTxProcessor // 自定义事务处理器
	HashFunc            rwsetutil.HashFunc                             // 哈希函数
}
