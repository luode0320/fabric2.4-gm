...
// GetPrivateData 方法是在 "ledger.Queryexecutor" 接口中实现的方法, 获取到的私有数据。
// 方法接收者：*queryExecutor
// 输入参数：
//   - ns：string 类型，表示命名空间。
//   - coll：string 类型，表示集合。
...
func (q *queryExecutor) GetPrivateData(ns, coll, key string) ([]byte, error) {
	...
	// 对于私有数据，元数据始终为零-因为元数据是哈希键的一部分 (而不是原始键)
	val, _, ver := decomposeVersionedValue(versionedValue)
	...
	if !version.AreSame(hashVersion, ver) {
		return nil, errors.Errorf(
			"匹配公共哈希版本的私有数据不可用. 公共哈希版本 = %s, 私有数据版本 = %s",
			hashVersion, ver,
		)
	}
	if q.collectReadset {
