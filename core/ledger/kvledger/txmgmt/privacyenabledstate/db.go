...
// StateDBConfig 封装了账本上的 stateDB 的配置信息。
type StateDBConfig struct {
	// ledger.StateDBConfig 用于配置账本上的 stateDB。
	*ledger.StateDBConfig
	// LevelDBPath 是当 statedb 类型为 "goleveldb" 时的文件系统路径。
	// 它由账本组件内部计算，
	// 因此它不在 ledger.StateDBConfig 中，也不对其他组件公开。
	LevelDBPath string
}

// DBProvider 封装了其他提供者，如 VersionedDBProvider 和 BookeepingProvider，这些提供者用于为通道创建数据库
type DBProvider struct {
	VersionedDBProvider statedb.VersionedDBProvider // 版本化数据库提供者
	HealthCheckRegistry ledger.HealthCheckRegistry  // 健康检查注册表
	bookkeepingProvider *bookkeeping.Provider       // 记账提供者
}

// NewDBProvider 构造 DBProvider 的实例。
// 输入参数：
//   - bookkeeperProvider：*bookkeeping.Provider，表示记账提供者
//   - metricsProvider：metrics.Provider，表示度量提供者
//   - healthCheckRegistry：ledger.HealthCheckRegistry，表示健康检查注册表
//   - stateDBConf：*StateDBConfig，表示状态数据库配置
//   - sysNamespaces：[]string，表示系统命名空间列表
//
// 返回值：
//   - *DBProvider：表示 DBProvider 的指针
...
func NewDBProvider(
	...
	dbProvider := &DBProvider{
		VersionedDBProvider: vdbProvider,         // 版本化数据库提供者
		HealthCheckRegistry: healthCheckRegistry, // 健康检查注册表
		bookkeepingProvider: bookkeeperProvider,  // 记账提供者
	}
...
// RegisterHealthChecker 向healthChecker注册基础stateDB。
// 现在，我们只注册CouchDB，因为它作为一个单独的进程运行，但不注册GoLevelDB，因为它是一个嵌入式数据库。
...
// GetDBHandle 获取给定id（即通道）的数据库句柄。
// 方法接收者：p（DBProvider类型的指针）
// 输入参数：
//   - id：通道的ID。
//   - chInfoProvider：通道信息提供者。
//
// 返回值：
//   - *DB：获取到的数据库句柄。
//   - error：如果获取数据库句柄时出错，则返回错误。
func (p *DBProvider) GetDBHandle(id string, chInfoProvider channelInfoProvider) (*DB, error) {
	// 使用 VersionedDBProvider 版本化数据库提供者获取数据库句柄, 检索所有配置块、命名空间和集合
	vdb, err := p.VersionedDBProvider.GetDBHandle(id, &namespaceProvider{chInfoProvider})
	if err != nil {
		return nil, err
	}

	// 使用bookkeepingProvider获取数据库句柄
...
// NewDB 创建一个DB实例，对VersionedDB实例进行封装。
// 方法接收者：无（函数）
// 输入参数：
//   - vdb：statedb.VersionedDB类型的实例，被封装的VersionedDB实例。
//   - ledgerid：账本ID。
//   - metadataHint：metadataHint实例，用于管理元数据信息。
//
// 返回值：
//   - *DB：创建的DB实例。
//   - error：如果创建DB实例时出错，则返回错误。
...
// GetChaincodeEventListener 返回一个实现了cceventmgmt.ChaincodeLifecycleEventListener接口的结构体，
// 前提是底层的statedb实现了statedb.IndexCapable接口。
// 方法接收者：s（DB类型的指针）
// 输入参数：无
// 返回值：
//   - cceventmgmt.ChaincodeLifecycleEventListener：实现了ChaincodeLifecycleEventListener接口的结构体。
func (s *DB) GetChaincodeEventListener() cceventmgmt.ChaincodeLifecycleEventListener {
	// 检查底层的VersionedDB是否实现了statedb.IndexCapable接口
...
// channelInfoProvider 接口使 privateenabledstate 包能够检索所有配置块、命名空间和集合。
type channelInfoProvider interface {
	// NamespacesAndCollections 返回通道的命名空间和集合。
	NamespacesAndCollections(vdb statedb.VersionedDB) (map[string][]string, error)
}

// namespaceProvider 实现statedb.NamespaceProvider接口
