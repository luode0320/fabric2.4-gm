...
type snapshotAdditionalMetadata struct {
	SnapshotHashInHex        string `json:"snapshot_hash"`          // 快照哈希（十六进制表示）
	LastBlockCommitHashInHex string `json:"last_block_commit_hash"` // 最后一个区块的提交哈希（十六进制表示）
}
...
func (l *kvLedger) generateSnapshot() error {
	...
	newHashFunc := func() (hash.Hash, error) {
		...
		return l.hashProvider.GetHash(common2.Hash())
	}
...
func (l *kvLedger) generateSnapshotMetadataFiles(
	...
	// 生成元数据哈希文件
	// todo luode 进行国密sm3的改造
	hash, err := l.hashProvider.GetHash(common2.Hash())
	if err != nil {
		return err
	}
...
func verifyFileHash(dir, file string, expectedHashInHex string, hashProvider ledger.HashProvider) error {
	// todo luode 进行国密sm3的改造
	hashImpl, err := hashProvider.GetHash(common2.Hash())
	if err != nil {
		return err
	}
