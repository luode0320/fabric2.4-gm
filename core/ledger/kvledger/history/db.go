...
// DBProvider 为给定通道提供HistoryDB句柄
type DBProvider struct {
	leveldbProvider *leveldbhelper.Provider // 允许将单个 LevelDB 用作多个逻辑 LevelDB
}

// NewDBProvider 实例化 DBProvider
func NewDBProvider(path string) (*DBProvider, error) {
	logger.Debugf("构造历史数据库 HistoryDBProvider dbPath=%s", path)

	// 创建leveldb, 并生成存储目录。
	levelDBProvider, err := leveldbhelper.NewProvider(
		&leveldbhelper.Conf{
			DBPath:         path,                     // 数据库存储目录
			ExpectedFormat: dataformat.CurrentFormat, // 内部数据库中格式键的预期值。
		},
	)
	if err != nil {
		return nil, err
	...
	// 通道提供HistoryDB句柄
	return &DBProvider{
		leveldbProvider: levelDBProvider, // 允许将单个 LevelDB 用作多个逻辑 LevelDB
	}, nil
}
