...
// collElgNotifier 监听链码事件，并确定对于一个或多个现有的私有数据集合，对等节点是否有资格，并通知已注册的监听器
type collElgNotifier struct {
	deployedChaincodeInfoProvider ledger.DeployedChaincodeInfoProvider // 链码信息提供者
	membershipInfoProvider        ledger.MembershipInfoProvider        // 成员信息提供者
	listeners                     map[string]collElgListener           // 注册的监听器映射，使用字符串作为键
}
