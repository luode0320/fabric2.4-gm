...
// Provider 提供对特定 'Store' 的句柄，该 'Store' 管理账本的私有写集
type Provider struct {
	dbProvider *leveldbhelper.Provider // LevelDB 提供者
	pvtData    *PrivateDataConfig      // 私有数据配置
}

// PrivateDataConfig 封装了账本上私有数据存储的配置
type PrivateDataConfig struct {
	// PrivateDataConfig 用于配置私有数据存储提供者
	*ledger.PrivateDataConfig // 私有数据配置

	StorePath string // 私有数据存储的文件系统路径
}

// Store 管理账本的私有写集的永久存储。
type Store struct {
	db              *leveldbhelper.DBHandle // 数据库句柄
	ledgerid        string                  // 账本ID
	btlPolicy       pvtdatapolicy.BTLPolicy // BTL策略
	batchesInterval int                     // 批处理间隔
	maxBatchSize    int                     // 最大批处理大小
	purgeInterval   uint64                  // 清理间隔

	isEmpty            bool              // 是否为空
	lastCommittedBlock uint64            // 最后一个提交的区块
	bootsnapshotInfo   *bootsnapshotInfo // 启动快照信息

	purgerLock      sync.Mutex       // 清理锁
	collElgProcSync *collElgProcSync // 集合资格处理同步器
	// 在提交旧区块的私有数据之后，
	// `isLastUpdatedOldBlocksSet` 被设置为 true。
	// 一旦状态数据库更新了这些私有数据，
	// `isLastUpdatedOldBlocksSet` 就会被设置为 false。
	// isLastUpdatedOldBlocksSet 主要用于恢复过程。
	// 在对等节点启动期间，如果 isLastUpdatedOldBlocksSet 设置为 true，
	// 则需要在完成恢复操作之前更新状态数据库中的私有数据。
	isLastUpdatedOldBlocksSet bool

	deprioritizedDataReconcilerInterval time.Duration // 降低优先级数据协调器的间隔
	accessDeprioMissingDataAfter        time.Time     // 访问降低优先级缺失数据的时间
}
...
func NewProvider(conf *PrivateDataConfig) (*Provider, error) {
	...
	// 提供对特定 'Store' 的句柄，该 'Store' 管理账本的私有写集
	return &Provider{
		dbProvider: dbProvider, // LevelDB 提供者
		pvtData:    conf,       // 私有数据配置
	}, nil
}
...
// LastCommittedBlockHeight 返回最后提交的块的高度
