...
func (d *deliverServiceImpl) StartDeliverForChannel(chainID string, ledgerInfo blocksprovider.LedgerInfo, finalizer func()) error {
	...
	if dc.BlockGossipDisabled {
		logger.Infow("此 peer 将从 orderer 服务中检索块(不会将它们传播给组织中的其他 peere 节点)", "channel", chainID)
	} else {
		logger.Infow("此 peer 节点将从 orderer 服务中检索块, 并传播给组织中的其他 peere 节点", "channel", chainID)
	}
