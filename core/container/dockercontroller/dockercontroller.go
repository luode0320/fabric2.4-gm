...
// DockerVM 是一个虚拟机。它由一个镜像 ID 来标识。
type DockerVM struct {
	PeerID          string             // 对等节点 ID
	NetworkID       string             // 网络 ID
	BuildMetrics    *BuildMetrics      // 构建指标
	HostConfig      *docker.HostConfig // 主机配置
	Client          dockerClient       // Docker 客户端
	AttachStdOut    bool               // 是否附加标准输出
	ChaincodePull   bool               // 是否拉取链码
	NetworkMode     string             // 网络模式
	PlatformBuilder PlatformBuilder    // 平台构建器
	LoggingEnv      []string           // 日志环境
	MSPID           string             // MSP ID
}
