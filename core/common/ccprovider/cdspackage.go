...
// CDSData 是在实例化链码时存储在 LSCC 中的数据，用于 CDSPackage。
// 这需要以 protobuf 格式进行序列化，以便用于 ChaincodeData。
type CDSData struct {
	// CodeHash 是 ChaincodeDeploymentSpec 中 CodePackage 的哈希值
	CodeHash []byte `protobuf:"bytes,1,opt,name=codehash,proto3"`

	// MetaDataHash 是 ChaincodeDeploymentSpec 中 Name 和 Version 的哈希值
...
// GetHasher 接口定义了包含GetHash函数的bccsp子集。
...
// CDSPackage 封装了 ChaincodeDeploymentSpec。
type CDSPackage struct {
	buf       []byte                      // 存储数据的字节切片
	depSpec   *pb.ChaincodeDeploymentSpec // 链码部署规范对象
	data      *CDSData                    // CDS 数据对象
	datab     []byte                      // 存储数据的字节切片
	id        []byte                      // 存储 ID 的字节切片
	GetHasher GetHasher                   // 获取哈希器的接口
}
...
func (ccpack *CDSPackage) GetId() []byte {
	// 这个操作必须在创建包并初始化之后进行
	// 如果这些步骤失败，就不应该调用 GetId()
	if ccpack.id == nil {
		panic("对未初始化的链码包调用了GetId")
	}
	return ccpack.id
}
