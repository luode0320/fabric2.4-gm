...
// connectionStore 是一个连接存储结构，用于管理与远程对等节点的连接。
type connectionStore struct {
	config           ConnConfig             // 连接配置
	logger           util.Logger            // 日志记录器
	isClosing        bool                   // 标识连接存储是否正在关闭
	shutdownOnce     sync.Once              // 确保只调用一次关闭方法
	connFactory      connFactory            // 创建与远程对等节点的连接
	sync.RWMutex                            // 用于同步访问共享变量
	pki2Conn         map[string]*connection // pkiID 到连接的映射
	destinationLocks map[string]*sync.Mutex // pkiID 到锁的映射，用于防止并发建立到同一远程节点的连接
}
