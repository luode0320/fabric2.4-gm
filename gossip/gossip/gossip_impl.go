...
// Node 是gossip网络的成员
type Node struct {
	selfIdentity          api.PeerIdentityType // 是同行的证书
	includeIdentityPeriod time.Time            // 导入身份的时间
	certStore             *certStore
	idMapper              identity.Mapper
	presumedDead          chan common.PKIidType // 定义保存pki-id的类型, 它是对等方的安全标识符
	disc                  discovery.Discovery   // 是表示发现模块的接口
	comm                  comm.Comm
	selfOrg               api.OrgIdentityType // 定义组织的身份
	*comm.ChannelDeMultiplexer
	logger            util.Logger     // 日志
	stopSignal        *sync.WaitGroup // 停止/等待/阻塞信号
	conf              *Config         // 是gossip组件的配置
	toDieChan         chan struct{}   //一个任意类型的结构体通道
	stopFlag          int32           // 停止标识
	emitter           batchingEmitter
	discAdapter       *discoveryAdapter
	secAdvisor        api.SecurityAdvisor // 定义提供安全和身份相关功能的外部辅助对象
	chanState         *channelState
	disSecAdap        *discoverySecurityAdapter
	mcs               api.MessageCryptoService // 是gossip组件与对等方的密码层之间的契约，对远程对等体及其发送的数据进行身份验证
	stateInfoMsgStore msgstore.MessageStore    // 将消息添加到内部缓冲区, 它可能会将消息添加到缓冲区
	certPuller        pull.Mediator            // 包装PullEngine的组件
	gossipMetrics     *metrics.GossipMetrics   // 封装所有gossip指标
}

// New 创建一个附加到 gRPC 服务器的 gossip 实例。启动心跳检测、检查存活、重新连接等协程。
//
// 输入参数：
//   - conf：配置信息。
//   - s：gRPC 服务器实例。
//   - sa：SecurityAdvisor 实例，用于提供安全建议。
//   - mcs：MessageCryptoService 实例，用于消息加密。
//   - selfIdentity：PeerIdentityType 实例，表示当前节点的身份。
//   - secureDialOpts：PeerSecureDialOpts 实例，用于安全拨号选项。
//   - gossipMetrics：GossipMetrics 实例，用于度量 gossip 相关的指标。
//   - anchorPeerTracker：AnchorPeerTracker 实例，用于跟踪锚节点。
//
// 返回值：
//   - *Node：Node 实例。
func New(conf *Config, s *grpc.Server, sa api.SecurityAdvisor,
	...
	// gossip网络的成员
	g := &Node{
		selfOrg:               sa.OrgByPeerIdentity(selfIdentity),                // 定义组织的身份
		secAdvisor:            sa,                                                // 定义提供安全和身份相关功能的外部辅助对象
		selfIdentity:          selfIdentity,                                      // 是同行的证书
		presumedDead:          make(chan common.PKIidType, presumedDeadChanSize), // 定义保存pki-id的类型, 它是对等方的安全标识符
		disc:                  nil,                                               // 是表示发现模块的接口
		mcs:                   mcs,                                               // 是gossip组件与对等方的密码层之间的契约，对远程对等体及其发送的数据进行身份验证
		conf:                  conf,                                              // 是gossip组件的配置
		ChannelDeMultiplexer:  comm.NewChannelDemultiplexer(),                    // 是一个可以接收通道注册的结构 (AddChannel) 和出版物 (deplexplex)，并根据其谓词将出版物广播给注册。
		logger:                lgr,                                               // 日志
		toDieChan:             make(chan struct{}),                               //一个任意类型的结构体通道
		stopFlag:              int32(0),                                          // 停止标识
		stopSignal:            &sync.WaitGroup{},                                 // 停止/等待/阻塞信号
		includeIdentityPeriod: time.Now().Add(conf.PublishCertPeriod),            // 导入身份的时间
		gossipMetrics:         gossipMetrics,                                     // 封装所有gossip指标
	}
	// 将消息添加到内部缓冲区, 它可能会将消息添加到缓冲区
	g.stateInfoMsgStore = g.newStateInfoMsgStore()
	//  保存pkiID(mspid+证书的哈希值)到对等体的证书 (标识) 之间的映射
	...
	commConfig := comm.CommConfig{
		DialTimeout:  conf.DialTimeout,  // 拨号超时 (单位: 秒)
		ConnTimeout:  conf.ConnTimeout,  // 连接超时 (单位: 秒)
		RecvBuffSize: conf.RecvBuffSize, // 收到消息的缓冲区大小
		SendBuffSize: conf.SendBuffSize, // 发送消息的缓冲区大小
	}

	// 创建一个新的 gossip 通信实例，将其自身绑定到给定的 gRPC 服务器
	...
	// MaxPropagationBurstSize 推送到远程对等方之前存储的最大消息数
	// MaxPropagationBurstLatency 连续推送消息之间的最大时间 (单位: 毫秒)
	// sendGossipBatch 将一批待发送的 Gossip 消息发送出去 回调函数
	g.emitter = newBatchingEmitter(conf.PropagateIterations,
		conf.MaxPropagationBurstSize, conf.MaxPropagationBurstLatency,
		g.sendGossipBatch)

	// 创建一个新的发现通信适配器。用于为发现模块提供所需的能力，这些能力在发现模块中的 comm 接口中声明。
	g.discAdapter = g.newDiscoveryAdapter()
	// 创建一个新的发现安全适配器。 用于为发现模块提供安全相关的能力。
	g.disSecAdap = g.newDiscoverySecurityAdapter()
	// 包含了发现模块的配置信息。
	discoveryConfig := discovery.DiscoveryConfig{
		AliveTimeInterval:            conf.AliveTimeInterval,            // 存活时间间隔，表示节点发送存活消息的时间间隔
		AliveExpirationTimeout:       conf.AliveExpirationTimeout,       // 存活过期超时时间，表示节点被认为已停止的超时时间
		AliveExpirationCheckInterval: conf.AliveExpirationCheckInterval, // 存活过期检查间隔，表示检查存活过期的时间间隔
		ReconnectInterval:            conf.ReconnectInterval,            // 重新连接间隔，表示节点重新连接的时间间隔
		MaxConnectionAttempts:        conf.MaxConnectionAttempts,        // 最大连接尝试次数，表示节点尝试连接的最大次数
		MsgExpirationFactor:          conf.MsgExpirationFactor,          // 消息过期因子，表示消息过期时间的因子
		BootstrapPeers:               conf.BootstrapPeers,               // 引导节点列表，表示用于引导发现的初始节点列表
	}
	// 返回当前节点的 NetworkMember 表示。
	self := g.selfNetworkMember()

	// 日志
	logger := util.GetLogger(util.DiscoveryLogger, self.InternalEndpoint)

	// 使用传递的 comm 模块和 crypto 服务创建一个新的发现服务。启动心跳检测、检查存活、重新连接等协程
	...
	g.logger.Infof("创建成员 Member 的 gossip 服务 %s", g.selfNetworkMember())
	...
	if g.conf.ExternalEndpoint == "" {
		g.logger.Warning("peer.gossip.externalEndpoint 外部节点为空，将无法在其组织外部访问peer对等点")
	}
...
// selfNetworkMember 返回当前节点的 NetworkMember 表示。
//
// 返回值：
//   - discovery.NetworkMember：当前节点的 NetworkMember 表示。
func (g *Node) selfNetworkMember() discovery.NetworkMember {
	self := discovery.NetworkMember{
		Endpoint:         g.conf.ExternalEndpoint, // 当前节点的外部端点地址
		PKIid:            g.comm.GetPKIid(),       // 当前节点的 PKI ID
		Metadata:         []byte{},                // 当前节点的元数据，默认为空
		InternalEndpoint: g.conf.InternalEndpoint, // 当前节点的内部端点地址
	}
	if g.disc != nil {
		self.Metadata = g.disc.Self().Metadata // 如果发现模块存在，则使用发现模块中当前节点的元数据
	}
	return self
}
...
func newChannelState(g *Node) *channelState {
	return &channelState{
		...
		channels: make(map[string]channel.GossipChannel), // 通道映射，存储通道名称与 GossipChannel 的映射关系
		g:        g,                                      // Gossip 节点
	}
}
...
// JoinChan 使gossip参与给定的通道，或更新它。
// 方法接收者：g（Node类型的指针）
// 输入参数：
//   - joinMsg：JoinChannelMessage类型，表示加入通道的消息。
//   - channelID：ChannelID类型，表示通道的ID。
func (g *Node) JoinChan(joinMsg api.JoinChannelMessage, channelID common.ChannelID) {
	...
	g.logger.Info("加入 gossip 网络通道", channelID, "与", len(joinMsg.Members()), "个 organizations 组织")
...
func (g *Node) learnAnchorPeers(channel string, orgOfAnchorPeers api.OrgIdentityType, anchorPeers []api.AnchorPeer) {
	if len(anchorPeers) == 0 {
		g.logger.Infof("没有为通道 %s 配置 peer 锚节点", string(orgOfAnchorPeers), channel)
		return
	}
	g.logger.Infof("正在连接 %s (通道 %s) 配置的 peer 锚节点: %v", string(orgOfAnchorPeers), channel, anchorPeers)
	for _, ap := range anchorPeers {
		if ap.Host == "" {
			g.logger.Warningf("获得了通道 %s 的空 Host 主机名, 正在跳过连接到 peer 锚节点 %v", channel, ap)
			continue
		}
		if ap.Port == 0 {
			g.logger.Warningf("为通道 %s 获得了无效端口 (0), 正在跳过连接到 peer 锚节点 %v", channel, ap)
			continue
		}
		...
		if g.selfNetworkMember().Endpoint == endpoint || g.selfNetworkMember().InternalEndpoint == endpoint {
			g.logger.Infof("具有相同节点的通道 %s 的 peer 锚节点, 正在跳过连接到自己", channel)
			continue
		}

		inOurOrg := bytes.Equal(g.selfOrg, orgOfAnchorPeers)
		if !inOurOrg && g.selfNetworkMember().Endpoint == "" {
			g.logger.Infof("peer 锚节点 %s:%d 不在我们的组织 (%v) 中, 并且我们没有外部节点, 正在跳过", ap.Host, ap.Port, string(orgOfAnchorPeers))
			continue
		}
		identifier := func() (*discovery.PeerIdentification, error) {
			...
			if err != nil {
				g.logger.Warningf("对节点 %s (通道 %s) 的深度探测失败: %s", endpoint, channel, err)
				return nil, err
			}
			...
			if bytes.Equal(orgOfAnchorPeers, g.selfOrg) && !isAnchorPeerInMyOrg {
				err := errors.Errorf("通道 %s 的锚节点 '%s' 不在我们的组织中, 但声明了这个节点是锚节点", channel, endpoint)
				g.logger.Warningf("%s", err)
				return nil, err
			}
			pkiID := g.mcs.GetPKIidOfCert(remotePeerIdentity)
			if len(pkiID) == 0 {
				return nil, errors.Errorf("无法提取标识为 %v 的远程 peer 节点的PKI ID", remotePeerIdentity)
			}
...
func (g *Node) start() {
	...
	g.logger.Info("gossip 实例", g.conf.ID, "已开始")
}
...
func (g *Node) sendGossipBatch(a []interface{}) {
	// 创建一个长度为 len(a) 的 msgs2Gossip 切片，用于存储待发送的 Gossip 消息
	msgs2Gossip := make([]*emittedGossipMessage, len(a))

	// 遍历 a 列表，将每个元素转换为 *emittedGossipMessage 类型，并将其存储到 msgs2Gossip 切片中
	for i, e := range a {
		msgs2Gossip[i] = e.(*emittedGossipMessage)
	}

	// 调用 g.gossipBatch 方法，将 msgs2Gossip 作为参数传递给该方法，发送批量的 Gossip 消息
...
// newDiscoveryAdapter 创建一个新的发现适配器。
//
// 返回值：
//   - *discoveryAdapter：新创建的发现适配器。
func (g *Node) newDiscoveryAdapter() *discoveryAdapter {
	// 用于为发现模块提供所需的能力，这些能力在发现模块中的 comm 接口中声明。
	return &discoveryAdapter{
		c:        g.comm,   // 通信器
		stopping: int32(0), // 停止标志
		// Gossip 消息处理函数
		gossipFunc: func(msg *protoext.SignedGossipMessage) {
			// 如果 PropagateIterations 为 0，则直接返回
			if g.conf.PropagateIterations == 0 {
				return
			}
			// 将 SignedGossipMessage 封装为 emittedGossipMessage，并添加到发送器中
			g.emitter.Add(&emittedGossipMessage{
				SignedGossipMessage: msg,
		...
		forwardFunc: func(message protoext.ReceivedMessage) {
			...
			// 将 ReceivedMessage 中的 GossipMessage 封装为 emittedGossipMessage，并添加到发送器中
			g.emitter.Add(&emittedGossipMessage{
				SignedGossipMessage: message.GetGossipMessage(),
				filter:              message.GetConnectionInfo().ID.IsNotSameFilter,
			})
		},
		incChan:          make(chan protoext.ReceivedMessage), // 接收消息通道
		presumedDead:     g.presumedDead,                      // 被假定为已停止的节点集合
		disclosurePolicy: g.disclosurePolicy,                  // 共享状态披露策略
	}
}

// discoveryAdapter 用于为发现模块提供所需的能力，这些能力在发现模块中的 comm 接口中声明。
type discoveryAdapter struct {
	stopping         int32                                       // 停止标志
	c                comm.Comm                                   // 通信器
	presumedDead     chan common.PKIidType                       // 被假定为已停止的节点集合
	incChan          chan protoext.ReceivedMessage               // 接收消息通道
	gossipFunc       func(message *protoext.SignedGossipMessage) // Gossip 消息处理函数
	forwardFunc      func(message protoext.ReceivedMessage)      // 转发消息处理函数
	disclosurePolicy discovery.DisclosurePolicy                  // 共享状态披露策略
}
...
// discoverySecurityAdapter 用于为发现模块提供安全相关的能力。
type discoverySecurityAdapter struct {
	identity              api.PeerIdentityType     // 节点身份
	includeIdentityPeriod time.Time                // 包含身份的时间段
	idMapper              identity.Mapper          // 身份映射器
	sa                    api.SecurityAdvisor      // 安全顾问
	mcs                   api.MessageCryptoService // 消息加密服务
	c                     comm.Comm                // 通信器
	logger                util.Logger              // 日志记录器
}

// newDiscoverySecurityAdapter 创建一个新的发现安全适配器。
//
// 返回值：
//   - *discoverySecurityAdapter：新创建的发现安全适配器。
func (g *Node) newDiscoverySecurityAdapter() *discoverySecurityAdapter {
	// 用于为发现模块提供安全相关的能力。
	return &discoverySecurityAdapter{
		sa:                    g.secAdvisor,            // 安全顾问
		idMapper:              g.idMapper,              // 身份映射器
		mcs:                   g.mcs,                   // 消息加密服务
		c:                     g.comm,                  // 通信器
		logger:                g.logger,                // 日志记录器
		includeIdentityPeriod: g.includeIdentityPeriod, // 包含身份的时间段
		identity:              g.selfIdentity,          // 节点身份
	}
}
...
func (g *Node) createCertStorePuller() pull.Mediator {
	// 定义了 pull mediator 的配置。
	conf := pull.Config{
		MsgType:           pg.PullMsgType_IDENTITY_MSG, // pull 消息的类型
		Channel:           []byte(""),                  // pull 的通道
		ID:                g.conf.InternalEndpoint,     // pull mediator 的唯一标识符
		PeerCountToSelect: g.conf.PullPeerNum,          // 用于发起 pull 的对等体数量
		PullInterval:      g.conf.PullInterval,         // pull 调用之间的时间间隔
		Tag:               pg.GossipMessage_EMPTY,      // pull 消息的标签
		PullEngineConfig: algo.PullEngineConfig{
			DigestWaitTime:   g.conf.DigestWaitTime,   // 摘要等待时间
			RequestWaitTime:  g.conf.RequestWaitTime,  // 请求等待时间
			ResponseWaitTime: g.conf.ResponseWaitTime, // 响应等待时间
		},
	}
	...
	certConsumer := func(msg *protoext.SignedGossipMessage) {
		...
		if idMsg == nil || idMsg.Cert == nil || idMsg.PkiId == nil {
			g.logger.Warning("无效的PeerIdentity:", idMsg)
			return
		}
		err := g.idMapper.Put(common.PKIidType(idMsg.PkiId), api.PeerIdentityType(idMsg.Cert))
		if err != nil {
			g.logger.Warningf("将pki-id与证书关联失败: %+v", errors.WithStack(err))
		}
		g.logger.Debug("得到一个新的新证书:", string(idMsg.Cert))
	}
	adapter := &pull.PullAdapter{
		Sndr:            g.comm,                      // 发送者，用于发送消息
		MemSvc:          g.disc,                      // 成员服务，用于获取成员信息
		IdExtractor:     pkiIDFromMsg,                // 标识符提取器，用于提取标识符
		MsgCons:         certConsumer,                // 消息消费者，用于消费消息
		EgressDigFilter: g.sameOrgOrOurOrgPullFilter, // 出站摘要过滤器，用于过滤出站摘要
	}
