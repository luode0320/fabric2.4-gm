...
// Config 定义了 pull mediator 的配置。
type Config struct {
	ID                string                   // pull mediator 的唯一标识符
	PullInterval      time.Duration            // pull 调用之间的时间间隔
	Channel           common.ChannelID         // pull 的通道
	PeerCountToSelect int                      // 用于发起 pull 的对等体数量
	Tag               gossip.GossipMessage_Tag // pull 消息的标签
	MsgType           gossip.PullMsgType       // pull 消息的类型
	PullEngineConfig  algo.PullEngineConfig    // pull 引擎的配置
}
...
// PullAdapter 定义了与 pullStore 交互的方法，以及与各种模块的 gossip 相关的属性。
type PullAdapter struct {
	Sndr             Sender              // 发送者，用于发送消息
	MemSvc           MembershipService   // 成员服务，用于获取成员信息
	IdExtractor      IdentifierExtractor // 标识符提取器，用于提取标识符
	MsgCons          MsgConsumer         // 消息消费者，用于消费消息
	EgressDigFilter  EgressDigestFilter  // 出站摘要过滤器，用于过滤出站摘要
	IngressDigFilter IngressDigestFilter // 入站摘要过滤器，用于过滤入站摘要
}

// Mediator 是包装PullEngine的组件，并提供方法
// 它需要执行拉同步。
// 将pull mediator专门化到某种类型的消息是由配置完成的，
// 一个标识符提取器，在构造时给出的标识符提取器，
// 以及可以为每种类型的pullMsgType (hello，digest，req，res) 注册的钩子。
type Mediator interface {
	// Stop 停止调解员
	Stop()

	// RegisterMsgHook 将消息挂钩注册到特定类型的拉取消息
	RegisterMsgHook(MsgType, MessageHook)

	// Add 将GossipMessage添加到中介器
	Add(*protoext.SignedGossipMessage)

	// Remove 从具有匹配摘要的中介器中删除GossipMessage，
	// 如果这样的消息退出
	Remove(digest string)

	// HandleMessage 处理来自某个远程对等点的消息
