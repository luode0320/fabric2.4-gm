...
func (mt *membershipTracker) checkIfPeersChanged(prevPeers discovery.Members, currPeers discovery.Members,
	...
	if !reflect.DeepEqual(wereInPrev, newInCurr) {
		if len(wereInPrev) == 0 {
			mt.report("组织成员已更改, peer 节点上线: ", newInCurr, ", 当前成员: ", currView)
		} else if len(newInCurr) == 0 {
			mt.report("组织成员已更改, peer 脱机: ", wereInPrev, ", 当前成员: ", currView)
		} else {
			mt.report("组织成员已更改, peer 脱机: ", wereInPrev, ", peer 上线: ", newInCurr, ", 当前成员: ", currView)
		}
	}
}
