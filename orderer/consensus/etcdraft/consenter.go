...
func (c *Consenter) detectSelfID(consenters map[uint64]*etcdraft.Consenter) (uint64, error) {
	...
	c.Logger.Warning("找不到", string(c.Cert), "among", serverCertificates)
...
func (c *Consenter) HandleChain(support consensus.ConsenterSupport, metadata *common.Metadata) (consensus.Chain, error) {
	...
	if c.EtcdRaftConfig.EvictionSuspicion == "" {
		c.Logger.Infof("未设置驱逐嫌疑 DefaultEvictionSuspicion, 默认为 %v", DefaultEvictionSuspicion)
	...
	if c.InactiveChainRegistry != nil {
		...
		c.Logger.Info("没有系统通道: 在驱逐 Registrar.SwitchToFollower 之后将被调用")
