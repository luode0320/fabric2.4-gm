...
// ReplicateChains 在给定的最后一个系统通道配置块的帮助下复制给定的链，并返回成功复制的链的名称。
...
type ChainReplicator interface {
	// ReplicateChains 使用给定的最后一个系统通道配置块复制给定的链。
	////返回复制成功的链的名称。
	ReplicateChains(lastConfigBlock *common.Block, chains []string) []string
}

// InactiveChainReplicator 跟踪禁用链并根据需要复制它们
