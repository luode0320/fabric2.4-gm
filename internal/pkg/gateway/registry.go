...
// registry 是一个注册表，用于管理背书节点和相关的连接信息。
type registry struct {
	...
	remoteEndorsers    map[string]*endorser   // 远程背书节点映射
	broadcastClients   sync.Map               // 排序节点地址 (string) -> 客户端连接 (orderer)
	channelInitialized map[string]bool        // 通道初始化状态映射
	configLock         sync.RWMutex           // 配置锁
	channelOrderers    sync.Map               // 通道 (string) -> 排序节点地址 (endpointConfig) 映射
	systemChaincodes   scc.BuiltinSCCs        // 系统链码
	localProvider      ledger.Provider        // 本地账本提供者
}
